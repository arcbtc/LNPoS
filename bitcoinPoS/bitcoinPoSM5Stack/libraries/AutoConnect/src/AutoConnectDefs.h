...
#endif // !AC_DEBUG_PORT
#ifdef AC_DEBUG
#define AC_DBG_DUMB(fmt, ...)                                \
  do                                                         \
  {                                                          \
    AC_DEBUG_PORT.printf_P((PGM_P)PSTR(fmt), ##__VA_ARGS__); \
  } while (0)
#define AC_DBG(fmt, ...)                                             \
  do                                                                 \
  {                                                                  \
    AC_DEBUG_PORT.printf_P((PGM_P)PSTR("[AC] " fmt), ##__VA_ARGS__); \
  } while (0)
#else
#define AC_DBG(...) \
  do                \
...
#if defined(ARDUINO_ARCH_ESP8266)
#define AUTOCONNECT_APID "bitcoinPoS"
#elif defined(ARDUINO_ARCH_ESP32)
#define AUTOCONNECT_APID "bitcoinPoS"
...
#define AUTOCONNECT_PSK "ToTheMoon"
...
#define AUTOCONNECT_MENU_TITLE "bitcoinPoS"
...
#define AC_ESP_LOG(t, l)     \
  do                         \
  {                          \
    esp_log_level_set(t, l); \
  } while (0)
#else
#define AC_ESP_LOG(...) \
  do                    \
